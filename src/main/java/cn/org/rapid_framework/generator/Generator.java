...
//		if(outRootDir == null) throw new IllegalStateException("'outRootDir' property must be not null.");
	...
	public Generator generateBy(Map templateModel,Map filePathModel) throws Exception {
		processTemplateRootDirs(templateModel, filePathModel,false, false);
		return this;
	}
     ...
     * @param templateModel 生成器模板可以引用的变量
     * @param filePathModel 文件路径可以引用的变量
     * @param mongoFlg 判断是否生成MongoDB的相关代码
     * @throws Exception
     */
    public Generator generateBy(Map templateModel,Map filePathModel, boolean mongoFlg) throws Exception {
        processTemplateRootDirs(templateModel, filePathModel,false, mongoFlg);
        return this;
    }
     ...
     * @throws Exception
     */
    public Generator deleteBy(Map templateModel,Map filePathModel, boolean mongoFlg) throws Exception {
        processTemplateRootDirs(templateModel, filePathModel,true, mongoFlg);
        return this;
    }
	
	private void processTemplateRootDirs(Map templateModel,Map filePathModel,boolean isDelete, boolean mongoFlg) throws Exception {
	    if(StringHelper.isBlank(getOutRootDir())) throw new IllegalStateException("'outRootDir' property must be not null.");
		...
		for(int i = 0; i < this.templateRootDirs.size(); i++) {
			...
			List<Exception> exceptions = scanTemplatesAndProcess(templateRootDir,templateModel,filePathModel,isDelete, mongoFlg);
			ge.addAll(exceptions); 
		}
		if(!ge.exceptions.isEmpty()) throw ge;
	}
	
	private List<Exception> scanTemplatesAndProcess(File templateRootDir, Map templateModel,Map filePathModel,boolean isDelete, boolean mongoFlg) throws Exception {
		...
		for(int i = 0; i < srcFiles.size(); i++) {
			...
			if (RapidUtil.isErrorValidPath(path, mongoFlg)) { //用于过滤不需要生成的目录
			    continue;
			}
			try {
			    if(isDelete){
	...
	private class TemplateProcessor {
		...
		public void executeGenerate(File templateRootDir,Map templateModel, Map filePathModel ,File srcFile) throws SQLException, IOException,TemplateException {
	    ...
	    public void executeDelete(File templateRootDir,Map templateModel, Map filePathModel ,File srcFile) throws SQLException, IOException,TemplateException {
	        String templateFile = FileHelper.getRelativePath(templateRootDir, srcFile);
            if(GeneratorHelper.isIgnoreTemplateProcess(srcFile, templateFile,includes,excludes)) {
                return;
            }
		...
		public void initGeneratorControlProperties(File srcFile) throws SQLException {
		...
		public void processTemplateForGeneratorControl(Map templateModel,String templateFile) throws IOException, TemplateException {
		...
		/** 处理文件路径的变量变成输出路径 */
		public String proceeForOutputFilepath(Map filePathModel,String templateFile) throws IOException {
		...
		public Template getFreeMarkerTemplate(String templateName) throws IOException {
			return GeneratorHelper.newFreeMarkerConfiguration(templateRootDirs, sourceEncoding,templateName).getTemplate(templateName);
		}
	
		public void generateNewFileOrInsertIntoFile( String templateFile,String outputFilepath, Map templateModel) throws Exception {
			Template template = getFreeMarkerTemplate(templateFile);
	...
	static class GeneratorHelper {
		...
		public static boolean isFoundInsertLocation(GeneratorControl gg,Template template, Map model, File outputFile, StringWriter newFileContent) throws IOException, TemplateException {
